...
## 2차원 배열 동적 메모리 할당 (포인터 배열을 사용해서 2 차원 배열 처럼 동작하는 배열을 만드는 방법)
```cpp
/* 2 차원 배열의 동적 할당 */
#include <stdio.h>
#include <stdlib.h>
...
## 2차원 배열 동적 메모리 할당 (실제로 2 차원 배열 크기의 메모리를 할당한 뒤 2 차원 배열 포인터로 참조하는 방법)
```cpp
#include <stdio.h>
#include <stdlib.h>
...
5 6 7 
```
* 동적 메모리를 할당해놓고 2차원 배열처럼 사용하는 것이다 
* 진짜 2차원 배열처럼 메모리가 연속적이다 
* 상대적으로 느린 malloc 함수를 딱 1번만 호출하면 된다 

## 노드(Node)
* <img src="./img/C_node.png" /> 
* 노드는 배열과는 달리 추가/삭제/삽입이 월등히 편리하지만, 예를들어 배열의 경우 3 번째 원소에 접근하기 위해서는 단순히 arr[3] 으로 하면 되지만 노드의 경우 헤드로 부터 3 번째 까지 일일히 찾아가야만 하는 단점이 있다 
* 추가/삭제/삽입이 자주 일어나는 경우 노드를 사용하고 특정한 번째에 찾아가야 하는 일이 잦은 일은 배열을 사용하는 것이 좋다
```cpp
#include <stdio.h>
#include <stdlib.h>
...
square(3) : 9 
```
* 매크로 함수는 단순히 square(3) 을 3*3 으로 치환하는 역할을 한다
* 주의할 점은 square(3 + 1) 은 3 + 1 * 3 + 1 이므로 결과가 예상했던 16이 아닌 7 이 되어 버린다 
* square(3 + 1) 을 예상했던 결과를 얻기 위해서는 #define square(x) (x) *(x) 와 같이 매크로 함수를 만들어야 한다 
